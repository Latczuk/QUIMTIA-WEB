...
export function Sidebar({ isMinimized, setIsMinimized, setActiveCategory, activeCategory }: any) {
      ...
      >
        <div className="p-4">
          ...
          <div className="flex justify-between items-center mb-6">
            {!isMinimized && (
              <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wider">Categorias</h2>
            )}
            <button
              ...
              type="button"
            >
              {isMinimized ? <Menu className="w-5 h-5 text-slate-600" /> : <X className="w-5 h-5 text-slate-600" />}
            </button>
          </div>

          <nav className="space-y-2" role="navigation" aria-label="Categorias de animais">
            {categories.map((category: category) => (
              ...
              >
                {isMinimized ? (
                  // Versão minimizada - apenas primeira letra ou ícone
                  <span className="font-bold text-sm">{category.label.charAt(0)}</span>
                ) : (
